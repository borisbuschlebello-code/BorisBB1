    ...
    while True:
        ...
        for prod in data:
            ...
            for v in (prod.get("variants") or []):
                ...
                # some stores leave SKU empty; fall back to variant id to keep it unique
                price_cents = None
                try:
                    price_cents = int(float(v.get("price","0"))*100)
                except Exception:
                    price_cents = None
                # variant image if present, else product image
...
def run():
    ...
    for url in KKIOSK_URLS:
        for p in kkiosk_shopify_items_all_variants(url):
            key=f"{p['site']}:{p['sku']}"
            old=state.get(key); img_hash=fetch_img_hash(p["image_url"]); now=int(time.time())
            if not old:
                changes.append(f"[NEW] {p['site']} · {p['sku']} · {p['name']} · {price_str(p['price_cents'])}")
                state[key]={**p,"image_hash":img_hash,"last_seen":now}
            else:
                if old.get("price_cents")!=p["price_cents"] and p["price_cents"] is not None:
                    changes.append(f"[PRICE] {p['site']} · {p['sku']} · {p['name']} · {price_str(old.get('price_cents'))} → {price_str(p['price_cents'])}")
                ...
                if img_hash and img_hash!=old.get("image_hash"):
                    ...
                    old["image_url"]=p["image_url"]; old["image_hash"]=img_hash
                old["last_seen"]=now; state[key]=old
    if changes:
        body="\n".join(changes); print(body); send_email("kkiosk watch: changes detected", body)
    else:
